...
- **Redesigned backoffice dashboard**: Clean, modern UI built with Lit (Umbraco 17/18) and AngularJS (Umbraco 13). Compact status legend, search bar, filters, bulk selection, tab-based navigation, frozen table headers, and click-to-sort columns all in one view. Validation and save errors are shown inside the Add/Edit redirect modal itself, so they're never hidden behind it.
